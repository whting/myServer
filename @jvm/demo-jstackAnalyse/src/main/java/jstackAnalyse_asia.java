...
public class jstackAnalyse_asia
{
    ...
    {
        // 当文件不存在只,读取特定目标文件
        File read = new File("E:\\jstack\\jstack.log");
        File write = new File("E:\\jstack\\chackThread.txt");
    ...
    public static void show(BufferedReader bufferedReader, BufferedWriter bufferedWriter) throws IOException, ParseException
    {
        ...
        {
            if (readerUp.contains("2013-")) // TODO jstack 本次,打印次数
            {
                catlogNum++;// jstack次数
                LogNum = 0;
            ...
            if (readerUp.contains("nid="))
            {
                ...
                String readerTeam = "[线程状态:" + threadStatus + " ] " + readerUp.substring(readerUp.indexOf("&quot;") + 6, readerUp.length());// TODO
                                                                                                                                            // 配合项目日志格式
