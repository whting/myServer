...
public class ThreadPool implements ThreadPoolDiagnose {
	...
	public ThreadPool(String thread_prefix) {
		...
		if (log.isDebugEnabled()) {
			log.debug("New pool.ThreadPool given: coresize=" + maxsize
	...
	public synchronized void start() {
		if (!running) {
			if(log.isDebugEnabled()) {
				log.debug("pool.ThreadPool.start() starting");
			}
			...
			if (log.isDebugEnabled()) {
				log.debug("pool.ThreadPool.start() is already running");
			}
		}
	}
	...
	public synchronized void stop() {
		if (running) {
			if(log.isDebugEnabled()) {
				log.debug("pool.ThreadPool.stop() stopping");
			}
			...
			if(log.isDebugEnabled()) {
				log.debug("pool.ThreadPool.stop() is already stop");
			}
		}
	}
	...
	public void execute(final Runnable torun) throws Exception {
		if (running && torun != null) {
			Runnable wapper = new Runnable() {
				public void run() {
					...
					try {
						...
						if (log.isDebugEnabled()) {
							log
									.debug("pool.ThreadPool.execute:torun="
											+ diagnose.className
											+ ", Elapsed Time="
			...
			if(log.isDebugEnabled()) {
				log.debug("pool.ThreadPool.execute() error:" + running+"="+running+", Runable="+torun);
			}
